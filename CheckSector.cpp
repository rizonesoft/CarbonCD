// CheckSector.cpp: CCheckSector ƒNƒ‰ƒX‚ÌƒCƒ“ƒvƒŠƒƒ“ƒe[ƒVƒ‡ƒ“
//
//////////////////////////////////////////////////////////////////////
...
using Mode1Frame = struct tMode1Frame
{
    BYTE Sync[12];
    BYTE Min;
    BYTE Sec;
...
CCheckSector::CCheckSector()
{
    ...
    //   create crc table for ECC
    {
        ...
        for (i = 1; i < 255; i++)
        {
            m_ParityCRC_Table[static_cast<BYTE>(i)] = static_cast<BYTE>(crc);
            m_ParityCRC_RevTable[static_cast<BYTE>(crc)] = static_cast<BYTE>(i);
    ...
    //   create intermediate transition table for ECC
    {
        ...
        for (j = 44; j >= 1; j--)
        {
            ...
            if (TmpMatrix[1][j] != 0)
            {
                TmpMatrix[0][j] = TmpMatrix[0][j] ^ m_ParityCRC_Table[(m_ParityCRC_RevTable[TmpMatrix[1][j]] + 1) %
                    255];
            }
        }
    ...
    for (j = 0; j < 43; j++)
    {
        for (i = 0; i < 24; i++)
        {
            for (k = 0; k < 2; k++)
            {
                ...
                {
                    m_EccP[j * 2 + 0 + k] ^= m_ParityCRC_Table[(m_ParityCRC_RevTable[buffer[i * 86 + k + j * 2]] +
                        m_ParityMatrix[0][i + 19]) % 255];
                    m_EccP[j * 2 + 86 + k] ^= m_ParityCRC_Table[(m_ParityCRC_RevTable[buffer[i * 86 + k + j * 2]] +
                        m_ParityMatrix[1][i + 19]) % 255];
                }
            }
        }
    }
    ...
    for (j = 0; j < 26; j++)
    {
        for (i = 0; i < 43; i++)
        {
            for (k = 0; k < 2; k++)
            {
                ...
                {
                    m_EccQ[j * 2 + 0 + k] ^= m_ParityCRC_Table[(m_ParityCRC_RevTable[buffer[(j * 86 + i * 88 + k) %
                        2236]] + m_ParityMatrix[0][i]) % 255];
                    m_EccQ[j * 2 + 52 + k] ^= m_ParityCRC_Table[(m_ParityCRC_RevTable[buffer[(j * 86 + i * 88 + k) %
                        2236]] + m_ParityMatrix[1][i]) % 255];
                }
            }
        }
    }
