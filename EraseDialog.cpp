...
{
    if (static_cast<CEraseDialog*>(Thread)->Erase())
    {
        static_cast<CEraseDialog*>(Thread)->Complete();
    }

    else
    {
        static_cast<CEraseDialog*>(Thread)->Incomplete();
    }
...
BOOL CEraseDialog::OnInitDialog()
{
    ...
    UpdateData(FALSE);
    m_ThreadID = 0;
    m_hThread = CreateThread(nullptr, 0, EraseThread, this, 0, &m_ThreadID);
