    ...
    for ( i = 0; i < max; i++ )
        {
            ...
            if ( lstrcmpi ( buf + lstrlen ( buf ) - 4, ".wav" ) == 0 )
                ...
                if ( lstrcmpi ( buf + lstrlen ( buf ) - 4, ".iso" ) == 0 )
                    {
                        InsertIsoTrack ( buf );
                    }

                else
                    if ( lstrcmpi ( buf + lstrlen ( buf ) - 4, ".bin" ) == 0 )
                        {
                            InsertIsoTrack ( buf );
                        }

                    else
                        if ( lstrcmpi ( buf + lstrlen ( buf ) - 4, ".img" ) == 0 )
                            {
                                InsertIsoTrack ( buf );
                            }
        }
            ...
            if ( memcmp ( Buffer, SyncHeader, 12 ) == 0 )
                {
                    ...
                    if ( Buffer[0x0c] == 0x00 && Buffer[0x0d] == 0x02 && Buffer[0x0e] == 0x00 && Buffer[0x0f] == 0x01 )
                        ...
                        if ( Buffer[0x0c] == 0x00 && Buffer[0x0d] == 0x02 && Buffer[0x0e] == 0x00 && Buffer[0x0f] == 0x02 )
                            {
                                //   mode2/2352
                                IsoType = 2;
                            }
